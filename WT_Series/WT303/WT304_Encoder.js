...
function milesightDeviceEncode(payload) {
    ...
    if ("temperature_control_valve_range" in payload) {
        encoded = encoded.concat(setTemperatureControlValveRange(payload.temperature_control_valve_range));
    }
    if ("valve_opening_range" in payload) {
        encoded = encoded.concat(setValveOpeningRange(payload.valve_opening_range));
    }
    if ("valve_control_interval" in payload) {
        encoded = encoded.concat(setValveControlInterval(payload.valve_control_interval));
    }
 ...
 * @param {number} temperature_control_valve_range range: [0, 15]
 * @example { "temperature_control_valve_range": 0 }
 */
function setTemperatureControlValveRange(temperature_control_valve_range) {
    var buffer = new Buffer(3);
    buffer.writeUInt8(0x7d);
    buffer.writeInt16LE(temperature_control_valve_range * 10);
    return buffer.toBytes();
}
 ...
 * @param {number} valve_opening_range.min range: [0, 100]
 * @param {number} valve_opening_range.max range: [0, 100]
 * @example { "valve_opening_range": { "min": 0, "max": 100 } }
 */
...
/**
 * Set valve control interval
 * @param {number} valve_control_interval unit: second, range: [1, 60]
 * @example { "valve_control_interval": 1 }
 */
function setValveControlInterval(valve_control_interval) {
    var buffer = new Buffer(2);
    buffer.writeUInt8(0x7f);
    buffer.writeUInt8(valve_control_interval);
    return buffer.toBytes();
}
 ...
 * @example { "temperature_source_config": { "source": 0, "missing_data_action": 0 } }
 */
