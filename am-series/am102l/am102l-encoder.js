...
function milesightDeviceEncode(payload) {
    ...
    if ("reporting_mode" in payload) {
        encoded = encoded.concat(setReportingMode(payload.reporting_mode));
    }
...
/**
 * reporting mode
 * @param {number} reporting_mode values: (0: From Now On, 1: Batch Reporting Mode)
 * @example { "reporting_mode": 1 }
 */
function setReportingMode(reporting_mode) {
    var mode_map = { 0: "From Now On", 1: "Batch Reporting Mode" };
    var mode_values = getValues(mode_map);
    if (mode_values.indexOf(reporting_mode) === -1) {
        throw new Error("reporting_mode must be one of " + mode_values.join(", "));
    }

    var buffer = new Buffer(3);
    buffer.writeUInt8(0xff);
    buffer.writeUInt8(0xcd);
    buffer.writeUInt8(getValue(mode_map, reporting_mode));
    return buffer.toBytes();
}
