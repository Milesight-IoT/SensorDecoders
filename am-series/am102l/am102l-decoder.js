...
function milesightDeviceDecode(bytes) {
    ...
    for (var i = 0; i < bytes.length; ) {
        ...
        // TEMPERATURE(2887)
        else if (channel_id === 0x03 && channel_type === 0xb5) {
            // °C
            decoded.report_interval = readReportInterval(bytes[i] & 0x01);
            decoded.collect_interval = readCollectInterval(bytes[i] >> 1 & 0x07);
            var times = bytes[i] >> 4 & 0x0f;
            decoded.collect_times = times;
            decoded.temperature = [];
            for(var j = 0; j < times; j++) {
                decoded.temperature.push(readInt16LE(bytes.slice(i + (2 * j + 1), i + (2 * j + 3))) / 10);
            }
            i += 1 + 2 * (times || 1);
        }
        // HUMIDITY
        ...
        // HUMIDITY(2887)
        else if (channel_id === 0x04 && channel_type === 0xb6) {
            decoded.report_interval = readReportInterval(bytes[i] & 0x01);
            decoded.collect_interval = readCollectInterval(bytes[i] >> 1 & 0x07);
            var times = bytes[i] >> 4 & 0x0f;
            decoded.collect_times = times;
            decoded.humidity = [];
            for(var j = 0; j < times; j++) {
                decoded.humidity.push(readUInt8(bytes[i + j + 1]) / 2);
            }
            i += 1 + (times || 1);
        }
        // HISTORY DATA
...
function handle_downlink_response(channel_type, bytes, offset) {
    ...
    switch (channel_type) {
        ...
        case 0x6a:
            ...
            break;
        case 0xcd:
            decoded.reporting_mode = readReportingMode(bytes[offset]);
            offset += 1;
            break;
        default:
...
function readReportInterval(type) {
    var interval_map = { 0: "60min", 1: "120min" };
    return getValue(interval_map, type);
}

function readCollectInterval(type) {
    var interval_map = { 1: "10min", 2: "15min", 3: "20min", 4: "30min", 5: "60min" };
    return getValue(interval_map, type);
}

function readReportingMode(type) {
    var mode_map = { 0: "From Now On", 1: "Batch Reporting Mode" };
