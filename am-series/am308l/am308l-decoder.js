...
function milesightDeviceDecode(bytes) {
    ...
    for (var i = 0; i < bytes.length; ) {
        ...
        else if (channel_id === 0x06 && channel_type === 0x9d) {
            decoded.light_value = readUInt16LE(bytes.slice(i, i + 2));
            i += 2;
        }
        // CO2
        ...
        // HISTORY DATA (AM308)
            ...
            data.pressure = readUInt16LE(bytes.slice(i + 16, i + 18)) / 10;
            data.pm2_5 = readUInt16LE(bytes.slice(i + 18, i + 20));
            data.pm10 = readUInt16LE(bytes.slice(i + 20, i + 22));
            i += 22;
        ...
        // RESPONSE DATA
            ...
            i = result.offset;
        } else if (channel_id === 0xf9) {
            var resultExt = handle_downlink_response_ext(channel_type, bytes, i);
...
function handle_downlink_response_ext(channel_type, bytes, offset) {
    ...
    switch (channel_type) {
        case 0xbb:
            decoded.light_unit = readUInt8(bytes[offset]);
            offset += 1;
            break;
        default:
            throw new Error("unknown downlink response");
