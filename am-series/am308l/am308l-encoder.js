...
function milesightDeviceEncode(payload) {
    ...
    if ("light_unit" in payload) {
        encoded = encoded.concat(setLightUnit(payload.light_unit));
    }
...
/**
 * light unit
 * @param {number} light_unit
 * @example { "light_unit": 1 }
 */
function setLightUnit(light_unit) {
    if (typeof light_unit !== "number") {
        throw new Error("light_unit must be a number");
    }
    if (light_unit !== 0 && light_unit !== 1) {
        throw new Error("light_unit must be 0 or 1");
    }

    var buffer = new Buffer(3);
    buffer.writeUInt8(0xf9);
    buffer.writeUInt8(0xbb);
    buffer.writeUInt8(light_unit);
    return buffer.toBytes();
}
