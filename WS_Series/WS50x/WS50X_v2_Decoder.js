...
function milesightDeviceDecode(bytes) {
    ...
    for (var i = 0; i < bytes.length; ) {
        ...
        // SWITCH STATE
            ...
            decoded.switch_1 = readOnOffStatus(bytes[i] & 0x01);
            decoded.switch_1_change = readYesNoStatus((bytes[i] >> 4) & 0x01);

            decoded.switch_2 = readOnOffStatus((bytes[i] >> 1) & 0x01);
            decoded.switch_2_change = readYesNoStatus((bytes[i] >> 5) & 0x01);

            decoded.switch_3 = readOnOffStatus((bytes[i] >> 2) & 0x01);
            decoded.switch_3_change = readYesNoStatus((bytes[i] >> 6) & 0x01);
            i += 1;
        }
        // RULE CONFIG
