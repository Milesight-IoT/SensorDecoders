...
function milesightDeviceEncode(payload) {
    ...
    if ("fetch_device_config" in payload) {
        encoded = encoded.concat(fetchDeviceConfig(payload.fetch_device_config));
    }
...
/**
 * fetch device config
 * @param {string|number} fetch_device_config values: ("report_interval", "distance_mode", "distance_range") or (1, 2, 3)
 * @example { "fetch_device_config": "report_interval" }
 * @example { "fetch_device_config": 1 }
 */
function fetchDeviceConfig(fetch_device_config) {
    var config_type_map = { 1: "report_interval", 2: "distance_mode", 3: "distance_range" };
    var config_type_values = getValues(config_type_map);
    var config_type = fetch_device_config;

    if (typeof fetch_device_config === "number") {
        if (!config_type_map[fetch_device_config]) {
            throw new Error("fetch_device_config must be one of 1, 2, 3");
        }
    } else {
        if (config_type_values.indexOf(fetch_device_config) === -1) {
            throw new Error("fetch_device_config must be one of " + config_type_values.join(", "));
        }
        config_type = getValue(config_type_map, fetch_device_config);
    }

    var buffer = new Buffer(3);
    buffer.writeUInt8(0xf9);
    buffer.writeUInt8(0x6f);
    buffer.writeUInt8(config_type);
    return buffer.toBytes();
}
