...
function handle_downlink_response_ext(code, channel_type, bytes, offset) {
    ...
    switch (channel_type) {
        ...
        case 0x39:
            ...
            break;
        case 0x6f:
            decoded.fetch_device_config = readFetchDeviceConfigType(readUInt8(bytes[offset]));
            offset += 1;
            break;
        case 0x8e:
            // support both 2-byte period and 1-byte ID + 2-byte period formats
            if (bytes[offset] === 0x00 && offset + 2 < bytes.length) {
                decoded.report_interval = readUInt16LE(bytes.slice(offset + 1, offset + 3));
                offset += 3;
            } else {
        ...
        case 0x1b:
            decoded.distance_range = {};
            decoded.distance_range.mode = readDistanceMode(readUInt8(bytes[offset]));
            decoded.distance_range.max = readUInt16LE(bytes.slice(offset + 1, offset + 3));
            offset += 3;
            break;
        default:
            throw new Error("unknown downlink response");
...
function readFetchDeviceConfigType(type) {
    var type_map = { 1: "report_interval", 2: "distance_mode", 3: "distance_range" };
