...
function syncTime(sync_time) {
    var sync_time_type_map = { 0: "no", 1: "yes" };
    var sync_time_type_values = getValues(sync_time_type_map);
    if (sync_time_type_values.indexOf(sync_time) === -1) {
        throw new Error("sync_time must be one of " + sync_time_type_values.join(", "));
    }

    if (getValue(sync_time_type_map, sync_time) === 0) {
        return [];
    }
...
function setPirWindowTime(pir_window_time) {
    var pir_window_time_map = { 0: 2, 1: 4, 2: 6, 3: 8 };
 ...
 * @param {object} dst_config.start_time
 * @param {number} dst_config.start_time.month, range: [1, 12]
 * @param {number} dst_config.start_time.week, range: [1, 5]
 * @param {number} dst_config.start_time.weekday, range: [1, 7]
 * @param {object} dst_config.end_time
 * @param {number} dst_config.end_time.month, range: [1, 12]
 * @param {number} dst_config.end_time.week, range: [1, 5]
 * @param {number} dst_config.end_time.weekday, range: [1, 7]
 ...
 * @param {number} d2d_master_config.mode values: (0: occupied, 1: vacant, 2: bright, 3: dim, 4: occupied and bright, 5: occupied and dim)
 ...
 * @param {number} hibernate_config.id values: (1: group_1, 2: group_2)
 * @param {number} hibernate_config.enable values: (0: disable, 1: enable)
 * @param {number} hibernate_config.start_time unit: minute, range: [0, 1440]
 * @param {number} hibernate_config.end_time unit: minute, range: [0, 1440]
...
function setHibernateConfig(hibernate_config) {
    ...
    var id_map = { 1: "group_1", 2: "group_2" };
    ...
    buffer.writeUInt8(getValue(id_map, id));
    buffer.writeUInt8(getValue(enable_map, enable));
