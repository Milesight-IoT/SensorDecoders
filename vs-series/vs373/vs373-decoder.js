...
function milesightDeviceDecode(bytes) {
    ...
    for (var i = 0; i < bytes.length; ) {
        ...
        // ODM(9543)
        else if (channel_id === 0x0d && channel_type === 0xb7) {
            decoded.people_count_status = readPeopleCountStatus(bytes[i]);
            decoded.period_people_count_status = readPeopleCountStatus(bytes[i + 1]);
            i += 4;
        }
        // ODM(9543)
        else if (channel_id === 0x0f && channel_type === 0xb8) {
            decoded.id = readUInt16LE(bytes.slice(i, i + 2));
            decoded.people_count_status = readPeopleCountStatus(bytes.slice(i + 2, i + 3));
            i += 4;
        }
        // ALARM
        ...
        // HISTORY DATA
            ...
            // EVENT TYPE: OUT OF BED
            if (alarm_type === 9) {
                data.people_count_status = readPeopleCountStatus(bytes[i + 7]);
                data.people_count = readUInt8(bytes[i + 8]);
            } else {
...
function handle_downlink_response_ext(code, channel_type, bytes, offset) {
    ...
    switch (channel_type) {
        ...
        // ODM(9543)
        case 0xc1: 
            decoded.people_count = readUInt8(bytes[offset]);
            offset += 1;
            break;
        // ODM(9543)
        case 0xc2:
            decoded.report_interval_with_second = readUInt32LE(bytes.slice(offset, offset + 4));
            offset += 4;
            break;
        // ODM(9543)
        case 0xc3:
            decoded.multiple_enable = readMultipleEnable(bytes[offset]);
            offset += 1;
            break;
        default:
            throw new Error("unknown downlink response");
...
function readPeopleCountStatus(status) {
    var people_count_status_map = { 
        0: "nobody", 
        1: "single", 
        2: "multiple" 
    };
    return getValue(people_count_status_map, status);
}
...
function readMultipleEnable(type) {
    var multiple_enable_map = { 0: "close", 1: "open" };
    return getValue(multiple_enable_map, type);
}
