...
function milesightDeviceEncode(payload) {
    ...
    if ("people_count" in payload) {
        encoded = encoded.concat(setPeopleCount(payload.people_count));
    }
    if ("report_interval_with_second" in payload) {
        encoded = encoded.concat(setReportIntervalWithSecond(payload.report_interval_with_second));
    }
    if ("multiple_enable" in payload) {
        encoded = encoded.concat(setMultipleEnable(payload.multiple_enable));
    }
 ...
 */
function setPeopleCount(people_count) {
    var buffer = new Buffer(3);
    buffer.writeUInt8(0xf9);
    buffer.writeUInt8(0xc1);
    buffer.writeUInt8(people_count);
    return buffer.toBytes();
}

/**
 * ODM(9543)
 * set report interval with second
 * @param {number} report_interval_with_second
 */
function setReportIntervalWithSecond(report_interval_with_second) {
    if(report_interval_with_second < 5) {
        throw new Error("report_interval_with_second must be >= 5");
    }

    var buffer = new Buffer(6);
    buffer.writeUInt8(0xf9);
    buffer.writeUInt8(0xc2);
    buffer.writeUInt32LE(report_interval_with_second);
    return buffer.toBytes();
}
 ...
 * ODM(9543)
 * set multiple enable
 * @param {number} multiple_enable values: (0: close, 1: open)
 */
function setMultipleEnable(multiple_enable) {
    var multiple_enable_map = { 0: "close", 1: "open" };
    var multiple_enable_values = getValues(multiple_enable_map);
    if (multiple_enable_values.indexOf(multiple_enable) === -1) {
        throw new Error("multiple_enable must be one of " + multiple_enable_values.join(", "));
    }

    var buffer = new Buffer(3);
    buffer.writeUInt8(0xf9);
    buffer.writeUInt8(0xc3);
    buffer.writeUInt8(getValue(multiple_enable_map, multiple_enable));
    return buffer.toBytes();
}
