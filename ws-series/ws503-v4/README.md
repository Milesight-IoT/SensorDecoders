...
| Switch Status            | 0xFF | 0x29 |   1    | switch_status1(0..1)+switch_status2(1..2)+switch_status3(2..3)<br/>switch_status1,switch_status2,switch_status3, values(0: off, 1: on) |
...
| D2D Agent Settings       | 0xFF | 0x83 |   5    | number(1B)+enable(1B)+control_command(2B)+action_status(1B)<br/>number, range[0, 15]<br/>enable, values(0: disable, 1: enable)<br/>action_status.switch, values(1: switch1, 2: switch2, 3: switch1,switch2, 4: switch3, 5: switch1,switch3, 6: switch2,switch3, 7: switch1,switch2,switch3)<br/>action_status.switch_status, values(0: off, 1: on, 2: reversal)                                                                                                                                                                                  |
| Time Zone                | 0xFF | 0xBD |   2    | time_zone(2B)                                                                                                                          |
| Schedule Settings        | 0xF9 | 0x64 |   7    | schedule_id(1B), range[1, 16]<br/>enable(0..4), values(1: enable, 2: disable), use_config(4..8), values(0, no, 1: yes)<br/>read: bits, (bit1: monday, bit2: tuesday, bit3: wednesday, bit4: thursday, bit5: friday, bit6: saturday, bit7: sunday)<br/>execut_hour(1B)<br/>execut_min(1B)<br/>switch_status1(0..2),switch_status2(2..4),switch_status3(4..6), values(0: keep, 1: on, 2: off, 3: reversal)<br/>lock_status(1B), values(0: keep, 1: lock, 2: unlock)                                                                                             |
| Get Schedule             | 0xF9 | 0x65 |   1    | schedule_id(1B), range[1, 16] & 255                                                                                                    |
| Load Power               | 0xF9 | 0xAB |   6    | switch_power1(2B),switch_power2(2B),switch_power3(2B), range[0, 1100]                                                                  |
| D2D Controller Settings  | 0xF9 | 0xB8 |   5    | keyid(1B)+key_contrl_enable(1B)+uplink(1B)+contrl_cmd(2B)<br/>keyid, values(0: key1, 1: key2, 2: key3)<br/>key_contrl_enable, values(0: disable, 1: enable)<br/>uplink.lora_enable(0..1), values(0: disable, 1: enable), uplink.key_enable(1..2), values(0: disable, 1: enable)                                                                |
...
// Switch Status (FF2974)
{
    "switch_status_control": {
        "switch_status1": "off",
        "switch_status2": "off",
        "switch_status3": "on"
    }
