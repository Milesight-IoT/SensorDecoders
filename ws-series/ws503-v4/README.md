...
| Button Status               | 0xFF | 0x29 |   1    | button_status1(0..1)+button_status2(1..2)+button_status3(2..3)+button_status1_change(4..5)+button_status2_change(5..6)+button_status3_change(6..7)<br/>button_status1,button_status2,button_status3, values(0: off, 1: on)<br/>button_status1_change,button_status2_change,button_status3_change, values(0: no, 1: yes)                   |
...
| D2D Agent Settings          | 0xFF | 0x83 |   5    | number(1B)+enable(1B)+control_command(2B)+action_status(1B)<br/>number, range[0, 15]<br/>enable, values(0: disable, 1: enable)<br/>action_status.button, values(1: button1, 2: button2, 3: button1,button2, 4: button3, 5: button1,button3, 6: button2,button3, 7: button1,button2,button3)<br/>action_status.button_status, values(0: off, 1: on, 2: reversal)                                                                                                                                                                                     |
| Time Zone                   | 0xFF | 0xBD |   2    | time_zone(2B)                                                                                                                          |
| Schedule Settings           | 0xF9 | 0x64 |   7    | schedule_id(1B), range[1, 16]<br/>enable(0..4), values(1: enable, 2: disable)<br/>use_config(4..8), values(0, no, 1: yes)<br/>read: bits, (bit1: monday, bit2: tuesday, bit3: wednesday, bit4: thursday, bit5: friday, bit6: saturday, bit7: sunday)<br/>execut_hour(1B)<br/>execut_min(1B)<br/>button_status1(0..2),button_status2(2..4),button_status3(4..6), values(0: keep, 1: on, 2: off, 3: reversal)<br/>lock_status(1B), values(0: keep, 1: lock, 2: unlock)                                                                                  |
| Get Schedule                | 0xF9 | 0x65 |   1    | schedule_id(1B), range[1, 16] & 255                                                                                                    |
| Power Consumption 2W        | 0xF9 | 0xAB |   7    | enable(1B)+button_power1(2B)+button_power2(2B)+button_power3(2B)<br/>enable, values(0: disable, 1: enable)<br/>button_power1,button_power2,button_power3, range[0, 1100]                                                                                                                                                                 |
| D2D Controller Settings     | 0xF9 | 0xB8 |   5    | button_id(1B)+contrl_enable(1B)+uplink(1B)+contrl_cmd(2B)<br/>button_id, values(0: button1, 1: button2, 2: button3)<br/>contrl_enable, values(0: disable, 1: enable)<br/>uplink.lora_enable(0..1), values(0: disable, 1: enable), uplink.button_enable(1..2), values(0: disable, 1: enable)                                                    |
| Daylight Saving Time        | 0xF9 | 0x72 |   9    | dst_bias(0..6)+enable(6..7)+start_month(1B)+start_week_num(0..3)+start_week_day(4..7)+start_hour_min(2B)+end_month(1B)+end_week_num(0..3)+end_week_day(4..7)+end_hour_min(2B)<br/>dst_bias, range[1, 120]<br/>enable, values(0: disable, 1: enable)<br/>start_month,end_month, values(1: Jan., 2: Feb., 3: Mar., 4: Apr., 5: May, 6: Jun., 7: Jul., 8: Aug. 9: Sep., 10: Oct., 11: Nov., 12: Dec.)<br/>start_week_num,end_week_num, values(1: 1st, 2: 2nd, 3: 3rd, 4: 4th, 5: last)<br/>start_week_day,end_week_day, values(1：Mon., 2：Tues., 3：Wed., 4：Thurs., 5：Fri., 6：Sat., 7：Sun.)<br/>start_hour_min,end_hour_min, range[0, 1440]                                                                                                          |
...
// Button Status (FF2974)
{
    "button_status_control": {
        "button_status1": "off",
        "button_status1_change": "yes",
        "button_status2": "off",
        "button_status2_change": "yes",
        "button_status3": "on",
        "button_status3_change": "yes"
    }
}
    ...
    "d2d_agent_settings_array": [
        {
            "action_status": {
                ...
                "button_status": "reversel"
            },
            "control_command": "aaaa",
            "enable": "enable",
            "number": 2
        }
    ]
...
// Power Consumption 2W (F9AB01580158015801)
{
