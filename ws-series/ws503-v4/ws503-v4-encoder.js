...
function milesightDeviceEncode(payload) {
    ...
    if("switch_status_control" in payload) {
        encoded = encoded.concat(setSwitchControl(payload.switch_status_control));
    }
 ...
 * @param {object} switch_status_control
 * @param {number} switch_status_control.switch_status1 values: (0: "off", 1: "on")
 * @param {number} switch_status_control.switch_status2 values: (0: "off", 1: "on")
 * @param {number} switch_status_control.switch_status3 values: (0: "off", 1: "on")
 * @example { "switch_status_control": { "switch_status1": 1, "switch_status2": 1, "switch_status3": 1 } }
 */
function setSwitchControl(switch_status_control) {
    var status_map = {0: "off", 1: "on"};
    var status_values = getValues(status_map);

    var data = 0x00;
    var switch_bit_offset = { switch_status1: 0, switch_status2: 1, switch_status3: 2 };
    for (var key in switch_bit_offset) {
        if (key in switch_status_control) {
            if (status_values.indexOf(switch_status_control[key]) === -1) {
                throw new Error("switch_status_control." + key + " must be one of: " + status_values.join(", "));
            }

            data |= 1 << (switch_bit_offset[key] + 4);
            data |= getValue(status_map, switch_status_control[key]) << switch_bit_offset[key];
        }
    }
 ...
 * @param {number} load_power.switch_power1
 * @param {number} load_power.switch_power2
 * @param {number} load_power.switch_power3
 * @example { "load_power": {"switch_power1": 0, "switch_power2": 0, "switch_power3": 1100 } }
 */
function setLoadPower(load_power) {
    var powers = [ "switch_power1", "switch_power2", "switch_power3" ];
 ...
 * @param {number} schedule_settings.switch_status1 values: (0: "keep", 1: "on", 2: "off", 3: "reversal")
 * @param {number} schedule_settings.switch_status2 values: (0: "keep", 1: "on", 2: "off", 3: "reversal")
 * @param {number} schedule_settings.switch_status3 values: (0: "keep", 1: "on", 2: "off", 3: "reversal")
 * @param {number} schedule_settings.lock_status values: (0: "keep", 1: "lock", 2: "unlock")
 * @example { "schedule_settings": [{ "schedule_id": 1, "enable": 1, "use_config": 1, "monday": 1, "tuesday": 0, "wednesday": 1, "thursday": 0, "friday": 1, "saturday": 0, "sunday": 1, "execut_hour": 10, "execut_min": 5, "switch_status1": 1, "switch_status2": 1, "switch_status3": 1, "lock_status": 1 }] }
 */
function setScheduleSettings(schedule) {
    ...
    var switch_bit_offset = { switch_status1: 0, switch_status2: 2, switch_status3: 4 };
 ...
 * @param {number} d2d_agent_settings_array.action_status.switch values: (1: "switch1", 2: "switch2", 3: "switch1, switch2", 4: "switch3", 5: "switch1, switch3", 6: "switch2, switch3", 7: "switch1, switch2, switch3")
 * @param {number} d2d_agent_settings_array.action_status.switch_status values: (0: "off", 1: "on", 2: "reversel")
 * @example { "d2d_agent_settings_array": [{"number": 0, "enable": 1, "control_command": "0000", "action_status": {"switch": 1, "switch_status": 1}}] }
 */
function setD2DAgentSettings(d2d_agent_settings) {
    ...
    var switchs = d2d_agent_settings.action_status.switch;
    var switch_status = d2d_agent_settings.action_status.switch_status;
    ...
    if (switch_values.indexOf(switchs) === -1) {
        throw new Error("d2d_agent_settings_array._item.action_status.switch must be one of " + switch_values.join(", "));
    }
    ...
    if (switch_status_values.indexOf(switch_status) === -1) {
        throw new Error("d2d_agent_settings_array._item.action_status.switch_status must be one of " + switch_status_values.join(", "));
    }
    ...
    data |= getValue(switch_map, switchs) << 4;
    buffer.writeUInt8(data);
    return buffer.toBytes();
}
