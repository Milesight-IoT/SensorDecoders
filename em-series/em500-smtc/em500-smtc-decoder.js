...
function milesightDeviceDecode(bytes) {
    ...
    for (var i = 0; i < bytes.length; ) {
        ...
        // SOIL MOISTURE (new resolution 0.1)
            ...
            var soil_moisture_value = readInt16LE(bytes.slice(i, i + 2));
            if (soil_moisture_value === 0xffff) {
                ...
                decoded.soil_moisture1 = readInt16LE(bytes.slice(i, i + 2)) / 10;
            }
            i += 2;
        }
            ...
            if (soil_moisture_value === 0xffff) {
                ...
                decoded.soil_moisture2 = readInt16LE(bytes.slice(i, i + 2)) / 10;
            }
            i += 2;
        }
            ...
            if (soil_moisture_value === 0xffff) {
                ...
                decoded.soil_moisture3 = readInt16LE(bytes.slice(i, i + 2)) / 10;
            }
            i += 2;
        }
        // CONDUCTIVITY (new resolution 0.01)
        else if (channel_id === 0x05 && channel_type === 0x7f) {
            var conductivity_value = readInt16LE(bytes.slice(i, i + 2));
            if (conductivity_value === 0xffff) {
                ...
                decoded.conductivity1 = readInt16LE(bytes.slice(i, i + 2)) / 100;
            }
            i += 2;
        }
            ...
            if (conductivity_value === 0xffff) {
                ...
                decoded.conductivity2 = readInt16LE(bytes.slice(i, i + 2)) / 100;
            }
            i += 2;
        }
            ...
            if (conductivity_value === 0xffff) {
                ...
                decoded.conductivity3 = readInt16LE(bytes.slice(i, i + 2)) / 100;
            }
            i += 2;
        }
        ...
        // HISTORY
            ...
            for(var j = 0; j < 3; j++) {
                ...
                if (temperature_value === 0xffff) {
                    ...
                    data['temperature' + (j+1)] = temperature_value / 10;
                }
    ...
    switch (channel_type) {
        ...
        case 0x06:
            ...
            if (id === 1 || id === 5 || id === 9) {
                ...
                decoded[key].threshold_min = readInt16LE(bytes.slice(offset + 1, offset + 3)) / 100;
                decoded[key].threshold_max = readInt16LE(bytes.slice(offset + 3, offset + 5)) / 100;
            }
            // temperature alarm
            ...
            // temperature mutation alarm
                ...
                decoded[key] = {};
                decoded[key].enable = readEnableStatus(enable);
                decoded[key].mutation = readInt16LE(bytes.slice(offset + 3, offset + 5)) / 10;
            }
            // soil_moisture alarm
