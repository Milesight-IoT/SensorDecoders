...
function setRuleConfig(rule_config) {
    ...
    var condition_data = writeCondition(condition);
    var action_data = [];
    for (var i = 0; i < action.length; i++) {
        action_data.push(writeAction(action[i]));
    }

    var buffer = new Buffer(209);
    buffer.writeUInt8(0xf9);
    buffer.writeUInt8(0x75);
    buffer.writeUInt8(data);
    buffer.writeBytes(condition_data);
    for (var i = 0; i < action_data.length; i++) {
        buffer.writeBytes(action_data[i]);
    }
    return buffer.toBytes();
}
...
function writeCondition(condition) {
    ...
    var condition_map = { 0: "none", 1: "time", 2: "value", 3: "modbus", 4: "message", 5: "d2d", 6: "reboot" };
    var condition_values = getValues(condition_map);
    if (condition_values.indexOf(condition_type) === -1) {
        throw new Error("condition.type must be one of " + condition_values.join(", "));
    }

    var buffer = new Buffer(49);
    var condition_value = getValue(condition_map, condition_type);
    buffer.writeUInt8(condition_value);
    switch (condition_value) {
        case 0x00:
            break;
        case 0x01:
            buffer.writeBytes(writeTimeCondition(condition.time_condition));
            break;
        case 0x02:
            buffer.writeBytes(writeModbusValueCondition(condition.modbus_value_condition));
            break;
        case 0x03:
            buffer.writeBytes(writeModbusCmdCondition(condition.modbus_cmd_condition));
            break;
        case 0x04:
            buffer.writeBytes(writeMessageCondition(condition.message_condition));
            break;
        case 0x05:
            buffer.writeBytes(writeD2DCondition(condition.d2d_condition));
            break;
        case 0x06:
            buffer.writeBytes(writeRebootCondition(condition.reboot_condition));
            break;
    }
...
function writeModbusValueCondition(modbus_value_condition) {
    ...
    var min = modbus_value_condition.min || 0;
    var max = modbus_value_condition.max || 0;
    ...
    var holding_mode_values = getValue(holding_mode_map, holding_mode);
    if (holding_mode_values.indexOf(holding_mode)) {
    ...
    if (condition === 2 && !("min" in modbus_value_condition) && !("continue_time" in modbus_value_condition) && !("lock_time" in modbus_value_condition)) {
    ...
    if (condition === 3 && !("max" in modbus_value_condition) && !("continue_time" in modbus_value_condition) && !("lock_time" in modbus_value_condition)) {
    ...
    if (condition === 4 && !("min" in modbus_value_condition) && !("max" in modbus_value_condition) && !("continue_time" in modbus_value_condition) && !("lock_time" in modbus_value_condition)) {
    ...
    if (condition === 5 && !("min" in modbus_value_condition) && !("max" in modbus_value_condition) && !("continue_time" in modbus_value_condition) && !("lock_time" in modbus_value_condition)) {
        throw new Error("rule_config._item.condition.modbus_value_condition.min and rule_config._item.condition.modbus_value_condition.max are required");
    }
    // change with time condition (mutation, mutation_duration)
    if (condition === 6 && !("mutation" in modbus_value_condition) && !("mutation_duration" in modbus_value_condition)) {
    ...
    if (condition === 7 && !("mutation" in modbus_value_condition)) {
    ...
    condition_data |= getValue(holding_mode_map, holding_mode) << 4;
    var value_1 = 0x00;
    var value_2 = 0x00;
...
/**
 * write reboot condition
 * @param {object} reboot_condition
 * @example { "reboot_condition": {} }
 */
function writeRebootCondition(reboot_condition) {
    var buffer = new Buffer(48);
    return buffer.toBytes();
}
...
function writeAction(action) {
    ...
    switch (action_type_value) {
        case 0x00:
            break;
        case 0x01:
            buffer.writeBytes(writeMessageAction(action.message_action));
            break;
        case 0x02:
            buffer.writeBytes(writeD2DAction(action.d2d_action));
            break;
        case 0x03:
            buffer.writeBytes(writeModbusAction(action.modbus_cmd_action));
            break;
        case 0x04:
            buffer.writeBytes(writeReportStatusAction(action.report_status_action));
            break;
        case 0x05:
            buffer.writeBytes(writeReportAlarmAction(action.report_alarm_action));
            break;
        case 0x06:
            buffer.writeBytes(writeRebootAction(action.reboot_action));
            break;
        default:
            throw new Error("rule_config._item.action.type must be one of " + action_values.join(", "));
    }
 ...
 * @param {string} message_action.message
...
function writeMessageAction(message_action) {
    ...
    if (message.length > 48) {
        throw new Error("message_action.message must be less than 48 characters");
    }
    if (!/^[0-9a-zA-Z,.;:!? ]+$/.test(message)) {
...
/**
 * write report status action
 * @param {object} report_status_action
 * @example { "report_status_action": {} }
 */
function writeReportStatusAction(report_status_action) {
    var buffer = new Buffer(48);
    return buffer.toBytes();
}
...
function writeReportAlarmAction(report_alarm_action) {
    ...
    // validate values
    var release_enable_map = { 0: "disable", 1: "enable" };
    var release_enable_values = getValues(release_enable_map);
    ...
    // encode
    var buffer = new Buffer(48);
    buffer.writeUInt8(getValue(release_enable_map, release_enable));
    return buffer.toBytes();
}

/**
 * write reboot action
 * @param {object} reboot_action
 * @example { "reboot_action": {} }
 */
function writeRebootAction(reboot_action) {
    var buffer = new Buffer(48);
    return buffer.toBytes();
}
...
Buffer.prototype.writeFloatLE = function (value) {
    this._write(toFloat32(value), 4, true);
    this.offset += 4;
};
...
function toFloat32(value) {
    ...
    switch (value) {
        ...
        default:
            ...
            var significant = ((value / Math.pow(2, exponent)) * 0x00800000) | 0;
