...
def decodeBytes(bytes):
    ...
    while i < len(bytes):
        ...
        # VALVE 1
            ...
            decoded['valve_1'] = "on" if bytes[i] else "off"
            i += 1
        # VALVE 2
        elif channel_id == 0x05 and channel_type == 0x01:
            decoded['valve_2'] = "on" if bytes[i] else "off"
            i += 1
        # VALVE 1 Pulse
        elif channel_id == 0x04 and channel_type == 0xc8:
            decoded['valve_1_pulse'] = int.from_bytes(bytes[i:i + 4], 'little', signed=False)
            i += 4
        # VALVE 2 Pulse
        ...
        # HISTORY
            ...
            valve = "on" if (bytes[i + 4] & 0x01)  else "off"
